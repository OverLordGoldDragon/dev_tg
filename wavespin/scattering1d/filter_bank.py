...
def scattering_filter_factory(N, J_support, J_scattering, Q, T,
    ...
    * k where k is an integer bounded below by 0. The maximal value for k is 'j'.
    ...
    * 'support': temporal support (interval outside which wavelet is ~0)
    # 'j': maximal value of k. Depends on the type of filter, it is dynamically
      chosen depending on max_subsampling and the characteristics of the filters.
...
def energy_norm_filterbank(psi_fs0, psi_fs1=None, phi_f=None, J=None, log2_T=None,
    ...
    def norm_filter(psi_fs, peak_idxs, lp_sum, n, s_idx=0):
        ...
        err_common_txt = ("Something likely went wrong before this function's "
                          "call (is Q too low and r_psi too great?)")
...
def compute_bandwidth(pf, criterion_amplitude=1e-3, right_only=False,
                      left_only=False):
    """Compute bandwidth of `pf` as interval, in number of samples, where
    energy inside the interval is `(1/criterion_amplitude)**2` times greater
    than energy outside the interval.
    ...
    criterion_amplitude : float
        Computes `criterion_energy` as `(1/criterion_amplitude)**2`.
        The `1/` is just for code clarity, what matters is the `**2`.

    right_only : bool (default False)
        Whether return only right half of bandwidth. Useful for `analytic=True`.

    left_only : bool (defualt False)
        `right_only` but left.
    ...
    pf = pf.squeeze()
    N = len(pf)
    # center on n=0 to handle left & right separately
    pf = np.roll(pf, -np.argmax(np.abs(pf)))
    epf = np.abs(pf)**2

    # find first point where ratio of energy up to that point to
    # the rest of energy is `criterion_amplitude**2`.
    def compute_ratios(epf):
        bound = N//2
        csum = np.cumsum(epf)
        # same as `epf[:i].sum() / epf[i:bound+1].sum()`
        e_ratios = [(csum[i - 1] /
                     ((csum[bound + 1] - csum[i - 1]) + 1e-9))
                    for i in range(1, bound + 1)]
        return np.array(e_ratios)

    er_right = compute_ratios(epf)
    # freq-reverse
    epf[1:] = epf[1:][::-1]
    er_left = compute_ratios(epf)

    # take half since we do left and right separately
    criterion_energy = (1 / criterion_amplitude)**2 / 2
    bw_right = np.where(er_right > criterion_energy)[0]
    bw_left  = np.where(er_left  > criterion_energy)[0]
    bw_right = bw_right[0] if len(bw_right) > 0 else 0
    bw_left  = bw_left[0]  if len(bw_left)  > 0 else 0
