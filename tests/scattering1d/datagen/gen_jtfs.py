...
for test_num in range(len(test_params)):
    ...
    np.savez(f"test_jtfs_{test_num}.npz", **params, x=x, **coeffs, **meta_flat,
             code=code)
...
params = dict(shape=2048, J=10, Q=16, J_fr=3, Q_fr=1, F=4, max_pad_factor=1,
...
coeffs = pack_coeffs(out, tp)
meta_flat = pack_meta(meta)
np.savez(f"test_jtfs_{test_num + 1}.npz", **params, x=x, **coeffs, code=code)
